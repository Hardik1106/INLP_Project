...
_sc = os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', 'sparse_coding')
if os.path.isdir(_sc):
    sys.path.insert(0, _sc)
from device_utils import get_device, empty_cache, get_model_dtype
...
def build_gsm8k_lookup(split="train"):
    ds = load_dataset("openai/gsm8k", "main", split=split, verification_mode="no_checks")
    table = {}
    for ex in ds:
...
for chunk_idx, (act_f_nc, act_f_c, ids_f) in enumerate(
    ...
    acts_nc = torch.load(act_f_nc, map_location="cpu").to(torch.float32).to(device)
    acts_c = torch.load(act_f_c, map_location="cpu").to(torch.float32).to(device)
    ...
    for local_idx, ids in enumerate(ids_list):
        ...
        def hook_fn(resid_pre, hook):
            patch = patched_act.to(resid_pre.dtype).to(resid_pre.device).reshape(1, 1, -1)
            return torch.cat([resid_pre[:, :-1, :], patch], dim=1)
