...
def run_direction(acts_from_dir, acts_to_dir, sae_from, sae_to, tag):
    ...
    for f_f, f_t, f_i in zip(files_f, files_t, files_i):
        if smp >= args.max_samples:
            break
        acts_f = torch.load(f_f, map_location="cpu").to(torch.float32).to(device)
        acts_t = torch.load(f_t, map_location="cpu").to(torch.float32).to(device)
        ...
        for ids in ids_ls:
            ...
            for K in K_LIST:
                ...
                def hk(resid, hook, _p=patched):
                    patch = _p.to(resid.dtype).to(resid.device).reshape(1, 1, -1)
                    return torch.cat([resid[:, :-1, :], patch], dim=1)
