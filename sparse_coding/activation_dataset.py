...
def setup_data(
    ...
    if use_cot:
        sentence_dataset = make_sentence_dataset_with_cot(dataset_name, max_lines=max_lines, start_line=start_line)
    else:
    ...
    token_loader = DataLoader(tokenized_sentence_dataset, batch_size=MODEL_BATCH_SIZE, shuffle=shuffle, collate_fn=collate_fn)
    if baukit:
