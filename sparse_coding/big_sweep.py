...
def ensemble_train_loop(ensemble, cfg, args, ensemble_name, sampler, dataset, progress_counter):
    ...
    for i, batch_idxs in enumerate(sampler):
        ...
        except (torch.cuda.OutOfMemoryError if torch.cuda.is_available() else RuntimeError, RuntimeError):
...
def sweep(ensemble_init_func, cfg):
    ...
    if cfg.use_synthetic_dataset:
        cfg.device = cfg.device if hasattr(cfg, 'device') and cfg.device else ("cuda:0" if torch.cuda.is_available() else "cpu")
